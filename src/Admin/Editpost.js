...
useEffect(() => {
  ...
  var DB = "http://localhost:5000/Images/";
  ...
  const submitform = async (e) => {
    ...
    try {
      if(ischangefile){
       axios.delete(`http://localhost:5000/api/${props.location.state.image_link}`);
      }
      const res = await axios.put(
        `http://localhost:5000/api/posts/${props.location.state._id}`,
        neweditpost
      );
